...
/// Узнать путь установки copilot cli
1. CMD -> where copilot
```
...
Новостной агент был реализован, во-первых, для этого, а во-вторых — чтобы проверить гипотезу использования кастомных агентов и скиллов. 
...
В новостном агенте присутствуют нюансы, над которыми я буду потинечку работать. К примеру, хочется добавить вызов формирования новостей в планировщик задач Windows. При выполнении я всё равно получаю запросы на подтверждение доступа к внешним веб-страницам, а хотелось бы, чтобы всё происходило на 100% автономно. Ну и посмотрим на качество генерации новостей; возможно, позже будет добавлен скилл автоматического деплоя после создания страницы. 
