...
LLamaSharp - кроссплатформенная библиотека для работы с LLM на локальном окружении. Основана на [llama.cpp](https://github.com/ggerganov/llama.cpp), поддерживает работу на CPU и GPU.

1. Создаем новый проект `Console Application`. Фреймворк .NET 9.0
...
```
3. Устанавливаем `бэкенд` пакет. Так разработчики LLamaSharp называют нативную C++ библиотеку (нашу любимую [llama.cpp](https://lmcorner.net/ru/posts/local-llm/)).
```
...
---
P.S. Названия GGUF моделей, оказывается 🤭, тоже несут в себе очень важную информацию, прилагаю расшифровку для модели которую мы выбрали. 
